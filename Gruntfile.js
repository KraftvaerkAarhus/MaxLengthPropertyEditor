module.exports = function(grunt) {
  ...
  grunt.initConfig({
    ...
    editorPath: 'Umbraco/Views/prevalueeditors',

    concat: {
      dist: {
        src: [
          'app/scripts/controllers/maxlength.property.editor.controller.js'
        ],
        dest: '<%= dest %>/<%= basePath %>/js/maxlength.property.editor.js',
        nonull: true
      }
    },
    ...
    less: {
      dist: {
        ...
        files: {
          '<%= dest %>/<%= basePath %>/css/maxlength.property.editor.css': 'app/styles/maxlength.property.editor.less',
        }
      }
    },
    ...
    copy: {
      ...
      prevalueeditors: {
        expand: true,
        cwd: 'app/prevalueeditors/',
        src: '**',
        dest: '<%= dest %>/<%= editorPath %>/'
      },
  ...
  grunt.registerTask('default', 'Concat files, build Less & copy config & views', function(){
    validateTarget();
    grunt.task.run(['concat', 'less', 'copy:config', 'copy:views', 'copy:prevalueeditors']);
  });
